...
I work in DevOps and built this to pair infrastructure/CI-CD work with applied AI — specifically computer vision (OpenCV) and a lightweight segmentation model (MobileSAM) running behind a real deployment pipeline, not just a notebook demo. The repo is structured to show both halves: a production-shaped AWS pipeline, and a working ML inference path with real tradeoffs (model size vs. accuracy, sync vs. async processing, GPU cost).
